...
That decision — emit or abstain — sits downstream of the one step that most determines everything else: the query the agent issues to go find methodology in the first place. A query policy is trained online with GRPO directly against this pipeline's own terminal reward, using the pipeline's real successes and failures as the signal, rather than a hand-labeled set of "good queries." For how that reward is actually assembled out of implementation quality, statistical validity, and abstention correctness, see `docs/terminal-reward.html`.
